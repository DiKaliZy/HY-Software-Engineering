...
class Professor:
    ...
    def enterBang(self, bangIndex):
        InitializeManager.InitializeManager.getBang(bangIndex)
    ...
    def inputStud(self, studNo, studName, studPhone):
        Bang.Bang.newDataInput(studNo, studName, studPhone)

    #목적 : 학생 정보 수정(교수권한)
    #매개변수 : 학번, 학생이름, 학생번호, 학생팀번호
    def modStudInform(self, studNo, studName, studPhone, studTeamNo):
        Bang.Bang.updateList(studNo, studName, studPhone, studTeamNo)

    #목적 : 팀 생성 가능 조건 설정
    #매개변수 : 없음
    def switchOper(self):
        Bang.Bang.swithOnOff()

    #목적 : 팀 구성 인원 설정
    #매개변수 : 설정할 인원 수
    def setLimit(self, limit):
        Bang.Bang.setLimit(limit)

    #목적 : 방 생성
    #매개변수 : 방 DB, 방 이름
    def makeBang(self, file, name):
        InitializeManager.InitializeManager.makeNew(file, name)
...
class Student:
    ...
    def modMyInform(self, name, phoneNo):
        Bang.Bang.updateList(id, name, phoneNo)

    #목적 : 원하는 사람에게 팀 가입을 신청한다
    #매개변수 : 원하는 사람의 학번
    def wantJoin(self, to):
        TeamOrganizer.TeamOrganizer.storeCommand(id ,to, 0)
    ...
    def negative(self, to):
        TeamOrganizer.TeamOrganizer.storeCommand(id, to, 2)

    #목적 : 자신이 속한 팀에서 탈퇴
    #매개변수 : 팀장의 학번
    def quitTeam(self, to):
        TeamOrganizer.TeamOrganizer.storeCommand(id, to, 3)
